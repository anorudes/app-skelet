...
const redisServerSideClient = redis.createClient();
const redisApiClient = redis.createClient();
import { REDIS_SERVER_SIDE_BASE, REDIS_API_BASE } from '../../config/main';

redisServerSideClient.select(REDIS_SERVER_SIDE_BASE[process.env.NODE_ENV]);
redisApiClient.select(REDIS_API_BASE[process.env.NODE_ENV]);
...
const Cache = {
  ...
  async getFromCache(projectId, type, ...keys) {
    ...
    // Get type timestamp
    const timestampType = await redisApiClient.getAsync(`${redisTypeKey}_timestamp`);
    ...
    if (redisItemKey) {
      ...
      const timestampItem = await redisApiClient.getAsync(`${redisFullKey}_${redisItemKey}_timestamp`);
    ...
    const data = await redisApiClient.getAsync(redisFullKey);
  ...
  async putToCache(data, projectId, type, ...keys) {
    ...
    if (redisItemKey) {
      ...
      redisApiClient.setAsync(redisFullKey,
  ...
  clearAllRedisServerSideCache() {
    // Clear all redis app server-sider-rendering cache
    return redisServerSideClient.flushdb();
  },
  ...
  _updateTypeTimestamp(redisTypeKey) {
    ...
    redisApiClient.setAsync(redisFullTimestampKey, newTimestamp);

    return newTimestamp;
  },

  async _updateItemTimestamp(redisTypeKey, redisItemKey) {
    let timestampType = await redisApiClient.getAsync(`${redisTypeKey}_timestamp`);
    const newTimestamp = Date.now();
    ...
    if (timestampType) {
      ...
      redisApiClient.setAsync(`${redisTypeKey}_timestamp`, newTimestamp);
